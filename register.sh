...
# https://docs.github.com/en/rest/actions/self-hosted-runners#create-a-registration-token-for-an-enterprise
...
if [ -z "${RUNNER_TOKEN:-}" ]; then
    if [ -z "${GITHUB_REPOSITORY:-}" ]; then
        # This block used to cover Org-scoped runners, now it will create Enterprise-scoped runners instead
        # GITHUB_OWNER env var must be set to the name of the Enterprise for this to work
        echo "Runner is scoped to enterprise '${GITHUB_OWNER}'"
        echo "View runner status at https://${GITHUB_DOMAIN}/enterprises/${GITHUB_OWNER}/settings/actions/runners"

        # token_url="https://${GITHUB_API_SERVER}/orgs/${GITHUB_OWNER}/actions/runners/registration-token"
        token_url="https://api.github.com/enterprises/${GITHUB_OWNER}/actions/runners/registration-token"
        registration_url="https://github.com/enterprises/vail-resorts"
    else
